...
//            }
    ...
    protected function process()
    {
        $wiz = $this->get_wizard();
        $action = $wiz->get_data('action');
        if ($action == 'install') { //TODO what if upgrade? e.g. don't show db elements ?
...
//          if( isset($_POST['db_type']) ) $this->_params['db_type'] = sanitizeVal($_POST['db_type'], ICMSSAN_PURE);
            ...
            // Stored in file and never displayed other than during this installer,
        ...
        $this->_params['timezone'] = sanitizeVal($_POST['timezone'], ICMSSAN_PUNCT); // any action

        try {
            $this->validate($this->_params);
            $wiz->merge_data('config', $this->_params);
            $url = $wiz->next_url();
            redirect($url);
    ...
    {
        $data = $this->get_wizard()->get_data('sessionchoices'); //TODO not defined until step 5
