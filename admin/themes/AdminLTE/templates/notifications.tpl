...
<li class="nav-item notifications">
	<a id="alerts"  class="nav-link">
		...
		{if $num_alerts}
			<span title="{lang('notifications_to_handle2', $num_alerts)}" class="badge badge-warning navbar-badge">{$num_alerts}</span>
		{else}
			<span title="{lang('notifications_to_handle2', $num_alerts)}"  class="badge navbar-badge">0</span>
		{/if}
	</a>
</li>
