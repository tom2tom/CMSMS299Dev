 ...
 *
 * Stylesheets can be organized into groups, this class manages the group itself.
 *
 * @package CMS
 * @license GPL
 * @since 2.3
...
class StylesheetsGroup
{
	 ...
	 * @ignore
	 */
	const TABLENAME = 'layout_css_categories';

	/**
	 * @ignore
	 */
	const MEMBERSTABLE  = 'layout_csscat_members';
	...
	*/
	...
	{
		if( !$this->get_name() ) throw new CmsInvalidDataException('A stylesheets group must have a name');
...
//			time()
		...
		$this->_dirty = false;
		audit($cid,'CMSMS','Stylesheets Group Created');
	}
...
//			time(),
	...
	public function delete()
	{
		...
		audit($cid,'CMSMS','Stylesheets Group Deleted');
		unset($this->_data['id']);
		$this->_dirty = true;
	}
	...
	public static function load($val)
	{
		...
		if( !$row ) throw new CmsDataNotFoundException('Could not find stylesheets group identified by '.$val);
	 ...
	 * @return array of StylesheetsGroup objects
	 */
