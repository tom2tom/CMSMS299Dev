  ...
  <div class="dashboard-box">
    <nav class="dashboard-inner">
      ...
      <h3 class="dashboard-icon {$node.name}">
        <a href="{$node.url}"{if isset($node.target)} target="{$node.target}"{/if}{if !empty($node.selected)} class="selected"{/if}><svg><use xlink:href="themes/Ghostgum/images/ggsprites.svg#{$node.name|lower}"/></svg> {$node.title}</a>
      </h3>
      {if $node.description}
