...
//			datePicker.fadeIn(150);
			...
			var datePickerOffset = {},
			 baseoff = opts.element.offset(),
			 verticalOrientation;

			datePicker.show();
			if (opts.orientation.match(/r/)) {
			  // right-align
			  datePickerOffset.left =
				baseoff.left +
				opts.element.outerWidth() -
				datePicker.outerWidth() +
				parseInt(datePicker.css("marginLeft").replace("px", ""), 10);
			} else {
			  // left-align
			  datePickerOffset.left =
				baseoff.left +
			...
			if (opts.orientation.match(/t/)) {
			  verticalOrientation = "t";
			} else if (opts.orientation.match(/b/)) {
			  verticalOrientation = "b";
			} else if (
			  baseoff.top + opts.element.outerHeight(true) + datePicker.outerHeight() >
			...
			if (verticalOrientation == "t") {
			  // position above
			  datePickerOffset.top =
				baseoff.top -
				datePicker.outerHeight() +
				parseInt(datePicker.css("marginTop").replace("px", ""), 10) - 2;
			} else {
			  // position below
			  datePickerOffset.top =
				baseoff.top +
				opts.element.outerHeight() +
				parseInt(datePicker.css("marginTop").replace("px", ""), 10) + 2;
			}
			datePicker.offset(datePickerOffset);
