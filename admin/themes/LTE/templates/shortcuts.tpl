<ul class="navbar-nav ml-auto">
  {if isset($myaccount)}
    <li>
		<a href="myaccount.php?{$secureparam}" class="nav-link" title="{'myaccount'|lang}"><i class="fa fa-user"></i></a>
	</li>
  {/if}
		    ...
		    {if $num_alerts > 0}
		        {if $num_alerts > 10}{$txt='&#2295'}{else}{$num=$num_alerts}{$txt="{$num}"}{/if}
 		        <li class="notifications">
					<a id="alerts" title="{_ld('admin','notifications_to_handle2',$num_alerts)}"><i class="fa fa-bell-o"></i><span class="bubble">{$txt}</span></a>
		        </li>
		    {/if}
		{/if}
		<li class="logout">
