...
(function ($) {
  $.widget('cmsms.filepicker', {
    ...
    _create: function () {
      ...
      this._settings.button = $('<button />').text(this._settings.btn_label).addClass('adminsubmit cmsfp cmsfp_choose cmsfp-choose');
...
//      value = this._relativePath( value, this.options.start_path );
    ...
    _create_popup: function () {
      ...
      this._settings.iframe = $('<iframe>').addClass('cmsfp cmsfp_frame')
       .attr({
      ...
      this._settings.container = $('<div>').addClass('cmsfp cmsfp_dlg cmsms-fp_dlg')
       .attr('title', title)
