...
function doSearch(files) {
 ...
 ob.find('i').on('click', function() {
  ...
  if(files) {
   ...
   $('#fm-tree .search-result').each(function() {
    var $el = $(this);
    if($el.is('ul')) {
     $el.find('li.tree-hidden').removeClass('tree-hidden');
    }
    $el.removeClass('tree-hidden search-result');
   });
   $('#fm-tree > li').removeClass('tree-hidden');
  }
 });
}
