...
<nav aria-title="breadcrumbs" class="float-sm-right">
	<ol class="breadcrumb">{$t=lang('home')}
		...
		{foreach $items as $one}
			...
			{$t=$one.title|adjust:'strip_tags'}
			{if !$one@last && !empty($one.url)}
				<a href="{$one.url}" title="{if !empty($one.description)}{$one.description|adjust:'strip_tags'}{else}{$t}{/if}">{$t}</a>
			{else}
				{$t}
			{/if}
			</li>
