...
function openup(el) {
 while(el !== null) {
  var nd = el.getAttribute('data-node') || null;
  if(nd) {
   var dat = JSON.parse(nd);
   if(!dat.open) {
    dat.open = 1;
    el.setAttribute('data-node', JSON.stringify(dat));
    el.className = el.className.replace('tree-closed', 'tree-opened'); //CHECKME if 'tree-empty'
    el = el.parent;
    if(el !== null) {
     nd = el.getAttribute('data-node') || null;
     if(nd) {
      dat = JSON.decode(nd);
      dat.opendown++;
      el.setAttribute('data-node', JSON.stringify(dat));
...
function closeup(el) {
 while(el !== null) {
  var nd = el.getAttribute('data-node') || null;
  if(nd) {
   var c, dat = JSON.parse(nd);
   if(dat.opendown > 0) {
    if(--dat.opendown === 0) {
     if(dat.open) {
      dat.open = 0;
      el.className = el.className.replace('tree-opened', 'tree-closed'); //CHECKME if 'tree-empty'
     }
    }
    el.setAttribute('data-node', JSON.stringify(dat));
   } else if(dat.open) {
    el.className = el.className.replace('tree-opened', 'tree-closed');
    dat.open = 0;
    el.setAttribute('data-node', JSON.stringify(dat));
   } else {
    return;
