...
{if $depth == 0}
<nav class="navigation" id="ac_menu" role="navigation">
 <ul id="ac_pagemenu">
{else}
 <ul>
...
{*TODO replace onclick handler*}
  <a href="{$navitem.url}" class="{$navitem.name} icon"{if isset($navitem.target)} target="_blank"{/if} title="{if !empty($navitem.description)}{$navitem.description|strip_tags}{else}{$navitem.title|strip_tags}{/if}" {if substr($navitem.url,0,6) == 'logout' && isset($is_sitedown)}onclick="cms_confirm_linkclick(this,'{$prompt}');return false;"{/if}>
 {if $depth > 0}{$navitem.title}{/if}
  </a>
 {if $depth == 0}
  {if !empty($navitem.children)}
  <span class="open-nav" title="{'open'|lang}/{'close'|lang} {$navitem.title|strip_tags}">{$navitem.title}</span>
  {else}
{*TODO replace onclick handler*}
