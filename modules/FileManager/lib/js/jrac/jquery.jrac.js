...
(function( $ ) {
  ...
  $.fn.jrac = function(options) {
    ...
    var defaults = $.fn.jrac.defaults;

    // Apply the resize and crop tools to each image
    return this.each(function() {
      ...
      var image_load_handler = function() {
        ...
        var $zoom_widget, $crop;
        if (jrac_loaded) {
          $zoom_widget = $container.find('div.jrac_zoom_slider');
          $crop = $viewport.find('div.jrac_crop');
        }
        else {
  ...
  // Default settings
  $.fn.jrac.defaults = $.extend({}, {
    'crop_width': 200,
    'crop_height': 100,
    // The two following properties define the crop position (relative to the image)
    'crop_x': 0,
    'crop_y': 0,
    'crop_resize': true,
