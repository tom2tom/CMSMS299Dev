<h3 class="pagesubtitle">{if $name!='-1'} {_ld('admin','edit_usrplg')}{else}{_ld('admin','add_usrplg')}{/if}</h3>
...
{/foreach}
  ...
  <div class="pageoverflow">
    {$t=_ld('admin','name')}* <label class="pagetext" for="name">{$t}:</label>
  ...
  <div class="pageoverflow">
    {$t=_ld('admin','description')}<label class="pagetext" for="description">{$t}:</label>
  ...
  <div class="pageoverflow">
    {$t=_ld('admin','parameters')}<label class="pagetext" for="parameters">{$t}:</label>
  ...
  {/if}
  <div class="pageoverflow">
    {$t=_ld('admin','code')}* <label class="pagetext" for="code">{$t}:</label>
  ...
  <div class="pageinput pregap">
    <button type="submit" name="submit" id="submitme" class="adminsubmit icon check">{_ld('admin','submit')}</button>
    <button type="submit" name="cancel" class="adminsubmit icon cancel">{_ld('admin','cancel')}</button>
    <button type="submit" name="apply" id="applybtn" title="{_ld('admin','title_apply_usrplg')}" class="adminsubmit icon apply">{_ld('admin','apply')}</button>
  </div>
</form>
