 ...
 * @property    Connection $db  A reference to the global database connnection object (deprecated)
 */
abstract class CMSModule
{
     ...
     *
     * The default behavior of this function is to include a file called method.uninstall.php
     ...
     * @param mixed  $returnid The current page id.  Int, or ''/null for admin requests.
     * @param mixed  $smarty A smarty-class object or Smarty_Internal_Template object.
     * @return string The action output.
     */
    final public function DoActionBase(string $name, string $id, array $params, $returnid = null, &$smarty)
    {
        $name = preg_replace('/[^A-Za-z0-9\-_+]/', '', $name);
        if( $returnid != '' ) {
        ...
        if( $gCms->template_processing_allowed() ) {
            $smarty = $gCms->GetSmarty();
            $tpl = $smarty->createTemplate( 'string:EMPTY MODULE ACTION TEMPLATE', null, null, $smarty );
     ...
     */
    public static function &GetModuleInstance(string $module)
    {
    ...
    /**
     * Store a message for ShowMessage() in the next request
     *
     * @since 1.11
     * @author Robert Campbell
    ...
    /**
     * Store a message for ShowErrors() in the next request
     *
     * @since 1.11
     * @author Robert Campbell
     * @param string|string[] $str The message.  Accepts either an array of messages or a single string.
