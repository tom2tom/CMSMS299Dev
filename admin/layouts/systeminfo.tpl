      ...
      <tr class="{cycle name='c4' values='row1,row2'}">
        ...
        <td style="width:24px">{if isset($test->res)}{admin_icon
           ...
           title={$test->res_text|default:''}
             alt={$test->res_text|default:''}}{/if}</td>
        <td>
          {if isset($test->value)}{$test->value}{else}&nbsp;{/if}{if isset($test->secondvalue)}({$test->secondvalue}){/if} {if isset($test->error_fragment)}
           <a href="{$cms_install_help_url}#{$test->error_fragment}" class="external" rel="external">
          {admin_icon icon='info-external.gif' title='?' alt='?'}
           </a>
          {/if}
      ...
      <tr class="{cycle name='c5' values='row1,row2'}">
        ...
        <td style="width:24px">{if isset($test->res)}{admin_icon
           ...
           title={$test->res_text|default:''}
             alt={$test->res_text|default:''}}{/if}</td>
        <td>
          {if isset($test->value) && $test->display_value}{$test->value}{else}&nbsp;{/if}{if isset($test->secondvalue)} ({$test->secondvalue}){/if}{if isset($test->error_fragment)}<a href="{$cms_install_help_url}#{$test->error_fragment}" class="external" rel="external">{admin_icon icon='info-external.gif' title='?' alt='?'}</a>{/if} {if isset($test->message)}{$test->message}{/if}
      ...
      <tr class="{cycle name='c6' values='row1,row2'}">
        ...
        <td style="width:24px">{if isset($test->res)}{admin_icon
           ...
           title={$test->res_text|default:''}
             alt={$test->res_text|default:''}}{/if}</td>
        <td>
          {if isset($test->value)}{$test->value}{else}&nbsp;{/if}{if isset($test->secondvalue)} ({$test->secondvalue}){/if}{if isset($test->message)}<br>{$test->message}{/if}
        </td>
      </tr>
      {/foreach} {/foreach}
    </tbody>
      ...
      <tr class="{cycle name='c7' values='row1,row2'}">
        ...
        <td style="width:24px">{if isset($test->res)}{admin_icon
           ...
           title={$test->res_text|default:''}
             alt={$test->res_text|default:''}}{/if}</td>
        <td>
          {if isset($test->value)}{$test->value}{else}&nbsp;{/if}{if isset($test->secondvalue)} ({$test->secondvalue}){/if}{if !empty($test->message)}<br>{$test->message}{/if}
          {if isset($test->opt)} {foreach $test->opt as $key => $opt}
          {if (!$opt@first || isset($test->value) || isset($test->secondvalue) || !empty($test->message))}<br>{/if}{admin_icon
            icon="icons/extra/{$opt.res|default:'space'}.gif"
           title={$opt.res_text|default:''}
