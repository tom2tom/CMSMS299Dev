...
<html lang="{$lang_code|truncate:'2':''}" dir="{$lang_dir|default:'ltr'}">
 ...
 <body>
  ...
  <div id="ggp_header">{*v boxchild and h container*}
     ...
     {if isset($sitelogo)}
      <div class="site-text">{_la('adminpaneltitle')}</div>
     {else}
      <span class="site-text">- {_la('adminpaneltitle')}</span>
     {/if}
     </div>

     <div>{*h boxchild *}
       <!-- logotext -->
       <span id="cms-text">{_la('power_by')}</span>
       <!-- logo -->
       <div id="cms-logo">
        <a href="http://www.cmsmadesimple.org" rel="external" title="{_la('cms_home')}">
        </a>
       </div>
      </div>
  ...
  <div id="ggp_container">{*v boxchild and h container*}
   <div id="ggp_navwrap" class="sidebar-on">
    <div id="ggp_navhead">
      <a href="#0" id="ggp_headlink" class="icon" title="{_la('open')}/{_la('close')}">
      ...
      <span id="ggp_headzone" title="{_la('close')}">&nbsp;</span>
    </div>
    <div id="ggp_nav">
