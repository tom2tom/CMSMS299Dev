...
use CMSMS\Utils;
use LogicException;
use HTMLEditor; // global namespace module class
...
class Profile implements ArrayAccess
{
	const KEYS = [
		...
		'menubar',
		'name',
//		'showstatusbar', not for this editor
		'system',
	];
...
//		case 'showstatusbar':
	...
	public function save()
	{
		if( !isset($this->_data['name']) || $this->_data['name'] == '' ) {
			throw new DataException('No name provided for HTMLEditor profile');
		}
	...
	public static function set_module(HTMLEditor $module)
	{
		self::$_module = $module;
	}
	...
	private static function _get_module()
	{
		if( !is_object(self::$_module) ) {
			self::$_module = Utils::get_module('HTMLEditor');
			if( !is_object(self::$_module) ) {
				// module not yet installed - hack for installation
				self::$_module = new HTMLEditor();
			}
		}
		return self::$_module;
	}
	...
	public static function load($name)
	{
		...
		if( !$data ) throw new UnexpectedValueException('Unknown HTMLEditor profile '.$name);
		$props = json_decode($data,true);
		if( !$props ) throw new RuntimeException('Invalid data for HTMLEditor profile '.$name);
