...
abstract class base_email_gateway
{
    ...
    public function __construct($mod)
    {
        if (!function_exists('curl_version')) {
            throw new RuntimeException("External email processing by CMSMailer requires PHP's cURL operations");
        }
        $this->mod = $mod;
        $this->gate_id = 0;
        self::reset();
...
/*    public function send()
    {
        ...
        if ($success) {
            ...
            $this->mod->Audit(CMSMailer::AUDIT_SEND, CMSMailer::MODNAME, $this->statusmsg);
        }
        return $success;
    }
*/
...
*/
    ...
    {
        $mod = $this->mod;
        $padm = $mod->CheckPermission('AdministerEmailGateways');
        $db = cmsms()->GetDb();
        $sql = 'SELECT * FROM '.CMS_DB_PREFIX.'module_cmsmailer_gates WHERE alias=?';
        if (!$padm) {
            $sql .= ' AND enabled=1';
        }
        ...
        $pmod = $padm || $mod->CheckPermission('ModifyEmailGateways');
        if (!($padm || $pmod)) {
...
*/
        ...
        $parms = [];
        $sql = 'SELECT gate_id,title,value,encvalue,apiname,signature,encrypt,enabled FROM '.CMS_DB_PREFIX.'module_cmsmailer_props WHERE gate_id=?';
        if (!$padm) {
            $sql .= ' AND enabled=1';
        }
    ...
    */
    public function handle_setup_form($params)
    {
        ...
        if ($delete) {
            ...
            $sql12 = 'DELETE FROM '.CMS_DB_PREFIX.'module_cmsmailer_props WHERE gate_id=? AND apiname=?';
        }
        ...
        $padm = $this->mod->CheckPermission('AdministerCMSMailerateways');
        $pw = PrefCrypter::decrypt_preference(PrefCrypter::MKEY);
        $o = 1;
        foreach ($conds as $apiname => &$data) {
            ...
            $data['encrypt'] = $enc;
            if ($enc) {
                $data['encvalue'] = Crypto::encrypt_string(''.$data['value'], $pw);
                $data['value'] = null;
            } else {
                $data['encvalue'] = null;
            }
            ...
            $sql = 'UPDATE '.CMS_DB_PREFIX.'module_cmsmailer_props SET '
    ...
    //$padm = boolean, TRUE if current user has AdministerCMSMailerateways permission
