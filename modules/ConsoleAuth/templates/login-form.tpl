<form action="{$loginurl}" enctype="multipart/form-data" method="post">
  ...
  {/if}
  <input type="text" name="{$actionid}{$usernamefld}"{if !isset($smarty.post.password)} class="focus"{/if} placeholder="{_ld($_module,'username')}" size="25" value="" autofocus>
  {/if}
  {if !$lost}
  <input type="password" name="{$actionid}password"{if isset($smarty.post.password) || isset($renewpw) || !empty($iserr)} class="focus"{/if} placeholder="{_ld($_module,'password')}" size="25" maxlength="72">
  {/if}
