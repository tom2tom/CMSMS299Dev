...
function CMSFileBrowser(_settings) {
  ...
  var gridview_btn = $('.filepicker-navbar-inner .view-grid');
  var listview_btn = $('.filepicker-navbar-inner .view-list');
  ...
  function _cmd_del(ev) {
    ...
    cms_confirm(settings.lang.confirm_delete).done(function() {
      _ajax_cmd('del', file).done(function(msg) {
        ...
        console.debug('filepicker command failed: ' + errorThrown);
      });
    });
  }
