...
{foreach $nodes as $node}
	...
	<div class="dashboard-box{if $node@index % 3 == 2} last{/if}">
		...
		{if !empty($node.img)}
			<a href="{$node.url}"{if isset($node.target)} target="{$node.target}"{/if}{if !empty($node.selected)} class="selected"{/if} tabindex="-1">
		...
		{/if}
			...
			{foreach $node.children as $one}
				<li><a href="{$one.url}"{if isset($one.target)} target="{$one.target}"{/if}{if substr($one.url,0,6) == 'logout' && isset($is_sitedown)} onclick="return confirm('{lang("maintenance_warning")|escape:"javascript"}');"{/if}>{$one.title}</a></li>
			{/foreach}
			</ul>
			{/if}
		</nav>
