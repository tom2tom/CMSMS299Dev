...
*/
$admin = AppState::test(AppState::ADMIN_PAGE);
$params = RequestParameters::get_action_params(true);

if ($params) {
	if (!isset($params['id'])) {
		$_REQUEST['id'] = ($admin) ? 'm1_' : 'cntnt01';
		$params = RequestParameters::get_action_params(true);
	}
//	if (defined('ASYNCLOG')) {
...
//	}
	...
	if ($modname) {
		...
		trigger_error('Missing module name.');
		exit;
	}
	$id = $params['id'] ?? (($admin) ? 'm1_' : 'cntnt01');
	$action = $params['action'] ?? (($admin) ? 'defaultadmin' : 'default');
	unset($params['id'], $params['action'], $params['module'], $params['inline']);
...
//			$template = $smarty->createTemplate('string:DUMMY PARENT');
			...
			$content = $mact_mod->DoActionBase($action, $id, $params, '', null);// no 'parent' tpl object
			...
			echo $mact_mod->DoActionBase($action, $id, $params, 0, null); // TODO frontend returnid  no 'parent' tpl object ?
//			if (!empty($params['module_error'])) handle it;
//			if (!empty($params['module_message'])) handle it;
		}
		break;
	case 1: // not full-page output
		if ($admin) {
			...
			$returnid = 0; //TODO frontend $returnid
		}
//		$smarty =  Lone::get('Smarty');
//		$template = $smarty->createTemplate('string:DUMMY PARENT');
		echo $mact_mod->DoActionBase($action, $id, $params, $returnid, null);
		break;
	case 2:	//minimal
//		if (defined('ASYNCLOG')) {
//			error_log('moduleinterface.php @2'."\n", 3, ASYNCLOG);
