...
*}
    ...
    <tbody>
      {foreach $stylesheets as $css}
        ...
        {$sid=$css->get_id()}
        {cms_action_url action='admin_edit_css' css=$sid assign='edit_css'}
{*
        {cms_action_url action='admin_copy_css' css=$sid assign='copy_css'}
        {cms_action_url action='admin_delete_css' css=$sid assign='delete_css'}
*}
    <tr class="{cycle values='row1,row2'}">
    {if !$css->locked()}
          <td><a href="{$edit_css}" data-css-id="{$sid}" class="edit_css tooltip" title="{$mod->Lang('edit_stylesheet')}" data-cms-description='{$css_tooltip}'>{$sid}</a></td>
          <td></td>
          <td><a href="{$edit_css}" data-css-id="{$sid}" class="edit_css tooltip" title="{$mod->Lang('edit_stylesheet')}" data-cms-description='{$css_tooltip}'>{$css->get_name()}</a></td>
        {else}
          <td>{$sid}</td>
    ...
    {if !$css->locked()}
{*
      <td><a href="{$edit_css}" data-css-id="{$sid}" class="edit_css" title="{$mod->Lang('edit_stylesheet')}">{admin_icon icon='edit.gif' title=$mod->Lang('edit_stylesheet')}</a></td>
...
*}
      <td>
        ...
        <input id="{$css@index}" type="checkbox" class="css_select" name="{$actionid}css_select[]" value="{$sid}"/>
      </td>
    {else}
{*
      <td>
        ...
        {if $lock.expires < $smarty.now}
          <a href="{$edit_css}" data-css-id="{$sid}" accesskey="e" class="steal_css_lock">{admin_icon icon='permissions.gif' class='edit_css steal_css_lock' title=$mod->Lang('prompt_steal_lock')}</a>
        {/if}
      </td>
      <td></td>
      <td></td>
