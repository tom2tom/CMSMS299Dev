 ...
 <tbody>
  ...
  {cycle values="row1,row2" name='cats' assign='rowclass'}
   <tr class="{$rowclass}" onmouseover="this.className='{$rowclass}hover';" onmouseout="this.className='{$rowclass}';">
    ...
    <td><a href="{$elem->edit_url}" title="{_ld($_module,'edit')}">{admin_icon icon='edit.gif'}</a></td>
    <td>{if $elem->delete_url}<a href="{$elem->delete_url}" title="{_ld($_module,'delete')}" class="del_cat">{admin_icon icon='delete.gif'}</a>{/if}</td>
   </tr>
  {/foreach}
 </tbody>
</table>
