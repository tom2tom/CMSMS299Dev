...
if ($data) {
	...
	foreach ($data as $row) {
		...
		if (!isset($types[$name])) {
			$db->Execute('INSERT INTO '.CmsLayoutTemplateType::TABLENAME.
			' (originator,name,owner,created,modified ) VALUES (?,?,-1,?,?)',
			[
				$name,
...
// DEBUG $sqlarray = $dbdict->DropTableSQL(CMS_DB_PREFIX.'module_templates');
// DEBUG $dbdict->ExecuteSQLArray($sqlarray);
