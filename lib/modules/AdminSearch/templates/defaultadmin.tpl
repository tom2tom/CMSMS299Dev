...
<div class="pageinput pageoverflow" id="filter_box">
  ...
  {foreach $slaves as $slave}
   <input class="filter_toggle" id="{$slave.class}" type="checkbox" value="{$slave.class}"{if !empty($saved_search.slaves) && in_array($slave.class,$saved_search.slaves)} checked="checked"{/if} title="{$slave.description}" />&nbsp;
   <label for="{$slave.class}">{$slave.name}</label><br />
  {/foreach}
  <br />
  <input type="checkbox" id="search_desc" value="1"{if !empty($saved_search.search_descriptions)} checked="checked"{/if} />&nbsp;
