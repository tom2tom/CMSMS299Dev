...
(function (factory) {
  ...
  DmUploaderFile.prototype.upload = function () {
    ...
    file.jqXHR.done(function(data, textStatus, jqXHR) {
        file.onSuccess(data);
    })
  ...
  DmUploaderFile.prototype.onError = function (jqXHR, textStatus, errorThrown) {
    ...
    if (this.status !== FileStatus.CANCELLED) {
      ...
      this.widget.settings.onUploadError.call(this.widget.element, this.id, jqXHR, textStatus, errorThrown);
    }
  };
  ...
  DmUploader.prototype.init = function () {
    ...
    if (input.length > 0) {
      ...
      var widget = this;
      // Or does it have the input as a child
  ...
  DmUploader.prototype.initDnD = function () {
    ...
    // -- Now our own Drop
    this.element.on("drop." + pluginName, function (evt) {
      evt.preventDefault();
    ...
    //-- These two events/callbacks are to maybe do some fancy visual stuff
    this.element.on("dragenter." + pluginName, function (evt) {
      evt.preventDefault();
    ...
    this.element.on("dragleave." + pluginName, function (evt) {
      evt.preventDefault();
  ...
  DmUploader.prototype.addFiles = function (files) {
    ...
    for (i = 0, n = files.length; i < n; i++) {
      ...
      if (this.settings.auto && !this.settings.queue) {
        if (nFiles === 0) {
          this.settings.onBegin.call(this.element);
        }
        file.upload();
      }
