(function() {
  var cmsms_linker = (function() {
    'use strict';
    var edobj;

    function linker_showDialog() {
      var data = {},
        selection = edobj.selection,
        dom = edobj.dom,
        selectedElm,
        anchorElm,
        initialText,
      ...
      function buildTargetList(targetValue) {
        ...
        if (!edobj.settings.target_list) {
          targetListItems.push({
            text: cmsms_tiny.target_new_window,
            value: '_blank'
          });
        }
        tinymce.each(edobj.settings.target_list, function(target) {
          targetListItems.push({
        ...
        function insertLink() {
          if (data.text !== initialText) {
            if (anchorElm) {
              ...
              edobj.insertContent(dom.createHTML('a', {
                href: href,
                target: data.target ? data.target : null,
                rel: data.rel ? data.rel : null,
                class: data.classname ? data.classname : null
            ...
            edobj.execCommand('mceInsertLink', false, {
              href: href,
              target: data.target,
              rel: data.rel ? data.rel : null,
              class: data.classname ? data.classname : null
      ...
      if (edobj.settings.target_list !== false) {
        targetOptions = {
          name: 'target',
          type: 'listbox',
    ...
    tinymce.util.Tools.resolve('tinymce.PluginManager').add('cmsms_linker', function(editor, pluginUrl) {
