...
//NOTE this assumes browser is ES5-capable
...
}.call(CMSMS_Admin, window, 'CMSMS_Admin'));
...
(function ($) {
   ...
   */
  var cms_helper = CMSMS_Admin.namespace('Helper');
  ...
  cms_helper.cms_resizableTextArea = function () {
    ...
    $('textarea').each(function () {
      var $this = $(this);
      //TODO do not assume 'MicroTiny'
      if ((typeof textarea.style.resize === 'undefined' || $this.hasClass('cms_resizable')) && (!$this.hasClass('MicroTiny')) && (!$this.hasClass('no-resize'))) {
        $this.resizable({
          handles: 'se',
          ghost: true
        });
   ...
   */
  var cms_loader = CMSMS_Admin.namespace('Loader');
...
/* TODO this causes duplication on jQuery 2.x WHY ?
    ...
    $(document).ajaxComplete(function() {
      cms_helper.cms_helpDialog();
      cms_helper.cms_initTooltips();
    });
*/
  };
  ...
  cms_loader.init = function () {
    cms_loader.reload();
    cms_helper.cms_resizableTextArea();
    cms_helper.cms_helpDialog();
    cms_helper.cms_initTabs();
    cms_helper.cms_initModalDialog();
    cms_helper.cms_initTooltips();
    cms_helper.cms_uploadLimit();
  };

  $(function () {
    cms_loader.init(); //CMSMS_Admin.Loader.init();
    // handle deprecated intra-request notifications
...
if (typeof cms_runtime === 'undefined') {
  var cms_runtime = false;
}
if (cms_runtime) {
  // unmunge (per https://github.com/felixmc/cabd.js/blob/master/cabd.js)
  var unscramble = function(str) {
    var l = str.length,
     o = (l & 1) ? 0 : 1,
     k = l - o,
     i, j,
    ...
    for (i = 0; i < l; i++) {
      j = (i & 1) ? (k + i) : (k - i - 1);
      result[i] = str[j / 2]; // ES5 syntax, no charAt() glitches
    }
    return result.join('');
  },
  plain = unscramble(decodeURIComponent(cms_runtime)),
  xvars;
  try {
    xvars = JSON.parse(plain);
  } catch (e) {
    xvars = {};
  }
  cms_data = $.extend({}, cms_data, xvars || {});
}
 ...
 * @param {Bool} Since 3.0 warn optional flag whether to pop up alert if key not found Default true
 */
function cms_lang(key, warn) {
  ...
  if (warn === undefined || warn) {
    cms_alert('Translation for \'' + key + '\' not set');
  }
  return '';
}
 ...
 * @param {Bool} state Optional new 'enabled' property of the buttons. Default true.
 */
function cms_button_able(btns, state) {
  var dis = (state !== undefined) ? !state : false;
  if (!(btns instanceof $ || Array.isArray(btns))) {
    btns = [btns];
  }
...
function cms_prompt(msg, suggest, title) {
  ...
  if (suggest) { suggest = suggest.replace(re, '<br>'); }
  if (title === undefined) title = '';
  if (title) { title = title.replace(re, '<br>'); }
 ...
 * @function cms_alert(msg, title, wait)
...
function cms_alert(msg, title, wait) {
  ...
  if (wait) {
    return cms_data.alertfunc(msg, title, true);
  } else {
    cms_data.alertfunc(msg, title, false);
  }
}
...
function cms_confirm(msg, title, yestxt, notxt) {
  ...
  if (!yestxt) yestxt = cms_lang('ok');
  if (!notxt) notxt = cms_lang('cancel');
 ...
 * @param {Bool} Optional flag Whether to enable or disable the busy state. Default true.
 */
function cms_busy(flag) {
  var $_div = $('#cms_busy');
  if ($_div.length === 0) {
    // gotta add one
    $_div = $('<div>').attr('id', 'cms_busy').addClass('busy')
    .hide().appendTo($('body'));
  }
  ...
  } else if ($_div.length) { // TODO $_div is visible
    $_div.hide();
  }
}
