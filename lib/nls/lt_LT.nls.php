<?php
#Native language name
$nls['language']['lt_LT'] = 'Lietuvių';
$nls['englishlang']['lt_LT'] = 'Lithuanian';

#Possible aliases for language
$nls['alias']['lt'] = 'lt_LT';
$nls['alias']['lithuanian'] = 'lt_LT';
$nls['alias']['lt_LT'] = 'lt_LT';
$nls['alias']['lt_LT.ISO8859-13'] = 'lt_LT';

#Possible locale for language
$nls['locale']['lt_LT'] = 'lt_LT.utf8,lt_LT.utf-8,lt_LT.UTF-8,lt_LT,lt_LT.ISO8859-13,lt_LT.ISO8859-4,lithuanian,Lithuanian_Lithuania.1257';

#Encoding of the language
$nls['encoding']['lt_LT'] = 'UTF-8';

#Location of the file(s)
$nls['file']['lt_LT'] = [__DIR__.'/lt_LT/admin.inc.php'];

#Language setting for HTML area
# Only change this when translations exist in HTMLarea and plugin dirs
# (please send language files to HTMLarea development)

$nls['htmlarea']['lt_LT'] = 'en';
